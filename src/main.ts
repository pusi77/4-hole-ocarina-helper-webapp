...
import {
  globalPerformanceTestRunner,
  PerformanceTestUtils,
} from './utils/PerformanceTestRunner.js';
  ...
  try {
      ...
      canvas,
      {
        ...
        onValidationChanged: async (result) => {
          ...
          const handlingResult =
            await errorHandlingManager.handleValidationResult(
              result,
              currentState,
              app.getText()
            );
    ...
    exportBtn.addEventListener('click', async () => {
      try {
        ...
        const result = await exportManager.exportCanvasToPNG(
          canvas,
          currentSong
        );
        ...
        if (result.success) {
          ...
          errorHandlingManager.handleExportSuccess(
            result.filename || 'chart.png',
            result.size || 0
          );
